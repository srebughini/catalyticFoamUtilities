...
do
	...
	echo " " >> controlDict
	echo "	sourceName       $name;" >> controlDict
	echo " " >> controlDict
...
rm -f log.cupMix
catalyticSimpleFoamCupMix > log.cupMix

rm -f G.txt
cat log.cupMix | grep "sum($name) of G" | awk '{print$5}' > G.txt

rm -f $name.txt
cat log.cupMix | grep "sum($name) of cup_mass_${name}" | awk '{print$5}' > $name.txt
