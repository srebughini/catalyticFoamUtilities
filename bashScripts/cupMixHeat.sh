...
do
	...
	echo '		('>> controlDict
	echo '			magU'>> controlDict
	echo '			gas::T'>> controlDict
	echo '		);'>> controlDict
	echo '	}' >> controlDict
done
echo '}' >> controlDict
...
foamCalc mag U > log
postProcessorCupMixForRhoSimpleFoam > log
...
echo "set output 'cupMixT.pdf'" >> plotResults
echo "unset label" >> plotResults
echo "unset key" >> plotResults
echo "set ylabel 'Temperature [K]'" >> plotResults
echo "set xlabel 'Axial length [mm]'" >> plotResults
echo "plot \\" >> plotResults
echo '"../cupMix.txt" u ($1*1000):($3/$4) w p linecolor rgb "#000000" pt 7 ps 0.5 title " ", \' >> plotResults
echo " " >> plotResults
echo " " >> plotResults
gnuplot plotResults
cd ..
